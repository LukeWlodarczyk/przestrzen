...
const baseClasses = tw("relative flex flex-grow flex-col");
...
const Textarea: FC<Props> = ({
  ...
  return (
    <div className={tw(baseClasses, className)}>
      ...
      <div className="noise-30 grow">
        <textarea
          aria-invalid={error ? "true" : "false"}
          aria-describedby={`${hintMessageId} ${error ? `${errorMessageId}` : ""}`}
          className={tw(
            "block h-full w-full resize-none rounded-xl border border-charcoal/10 bg-sand-beige/40 px-(--space-xs-s) py-(--space-2xs-xs) font-sans text-base font-extralight text-charcoal duration-300 outline-none focus:border-charcoal/40",
            errorClasses,
            validFilledClasses,
          )}
          id={id}
